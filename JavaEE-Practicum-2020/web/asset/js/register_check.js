...
function captchaOnBlur(event) {
    ...
    $.get(`getCaptcha?timestamp=${new Date().valueOf()}`,
        {},
        function(result) {
            if(!result["err_code"]) {
...
$().ready(function() {
    ...
    // Ajax验证用户名;
    username.on("blur", null, {index: 0}, function(event) {
        let index = event.data.index;
    ...
    // 验证密码;
    password.on("blur", null, {index: 1}, function(event) {
        let index = event.data.index;
    ...
    // 重复验证密码;
    rePassword.on("blur", null, {index: 2}, function(event) {
        let index = event.data.index;
        let passwordVal = password.val();
    ...
    // 验证手机号码;
    phoneNumber.on("blur", null, {index: 3}, function phoneNumberOnBlur(event) {
        let index = event.data.index;
    ...
    captchaImage.on("click", null, {captchaImage: captchaImage}, refreshCaptcha);
    captcha.on("focus", null, {captchaPrompt: captchaPrompt}, captchaOnFocus);
    captcha.on("blur", null, {index: 4, captcha: captcha, captchaPrompt: captchaPrompt}, captchaOnBlur);
    ...
    form.on("submit", function(event) {
        ...
        if (flag) {
            ...
            $.post("register",
                {username: username.val(), password: password.val(), phoneNumber: phoneNumber.val(), gender: $(`input[name="gender"]`).val()},
                function(result) {
                    if(!result["err_code"]) {
